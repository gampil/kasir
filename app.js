...
// ==========================================================================
const _0x17fdfa=_0x195d;function _0x3919(){const _0x56519d=['2043872IJTtIb','rhOxkgx/ex','1283188ZRLDFX','ForesaSecu','54ISWhNY','S0moyvH6GC','reToken202','os/s/AKfyc','110369amRipR','ript.googl','627wukTHj','https://sc','6_Xyz','4vtRpOiqRf','e.com/macr','zTioutoYdF','30rUdRQW','bwidT5jhAd','573715tVdopj','9d2NufVCbt','1199340dBkJCw','102uoqCmS','479844UlRrwq','0C5yc8quxu'];_0x3919=function(){return _0x56519d;};return _0x3919();}(function(_0x55c7c3,_0x132876){const _0x1f5ec8=_0x195d,_0x76120c=_0x55c7c3();while(!![]){try{const _0x2e58ef=-parseInt(_0x1f5ec8(0x73))/(-0x49*0x16+0x583+0xc4)*(parseInt(_0x1f5ec8(0x79))/(0x243*0x5+0x26a6+-0x31f3))+parseInt(_0x1f5ec8(0x7f))/(-0x13d*0xa+-0x132a+-0x1f8f*-0x1)+parseInt(_0x1f5ec8(0x6b))/(-0x3d9*0x1+0x15*0x19f+-0x1e2e)+-parseInt(_0x1f5ec8(0x7b))/(0x1443+-0x111f+-0x1*0x31f)+parseInt(_0x1f5ec8(0x7e))/(-0x17*-0xb1+-0x186e+0x88d)*(parseInt(_0x1f5ec8(0x71))/(-0x1*-0x223d+-0x1*0xc15+-0x1621))+parseInt(_0x1f5ec8(0x69))/(0x2*0xd84+-0x1d20+0x220)+-parseInt(_0x1f5ec8(0x6d))/(0x3b4+-0x2062+0x1cb7)*(parseInt(_0x1f5ec8(0x7d))/(0x1525+0xa5*0xb+0xe19*-0x2));if(_0x2e58ef===_0x132876)break;else _0x76120c['push'](_0x76120c['shift']());}catch(_0x550ad7){_0x76120c['push'](_0x76120c['shift']());}}}(_0x3919,0x17*-0x1136+-0x608*0x36+-0x86db*-0xa));function _0x195d(_0x20f0cd,_0x15fd29){_0x20f0cd=_0x20f0cd-(-0x28*-0x4f+-0x1b39*0x1+0xf49);const _0x411049=_0x3919();let _0x66be09=_0x411049[_0x20f0cd];return _0x66be09;}const MASTER_SCRIPT_URL=_0x17fdfa(0x74)+_0x17fdfa(0x72)+_0x17fdfa(0x77)+_0x17fdfa(0x70)+_0x17fdfa(0x7a)+_0x17fdfa(0x76)+_0x17fdfa(0x68)+_0x17fdfa(0x78)+_0x17fdfa(0x7c)+_0x17fdfa(0x6e)+_0x17fdfa(0x6a)+'ec',API_KEY=_0x17fdfa(0x6c)+_0x17fdfa(0x6f)+_0x17fdfa(0x75);
...
function bulatkanHarga(harga) {
    ...
    if (sisa < 400) return basis;            
    if (sisa >= 400 && sisa < 800) return basis + 500; 
    return basis + 1000;                     
}
...
// ===============================================================
// LOGIKA LOGIN MENGGUNAKAN MASTER DB (DENGAN API KEY)
// ===============================================================
async function syncMasterData() {
    if (!MASTER_SCRIPT_URL || MASTER_SCRIPT_URL.includes("TEMPEL_LINK")) return;
    try {
        // TAMBAHAN API KEY & ANTI-CACHE
        const response = await fetch(`${MASTER_SCRIPT_URL}?action=getMasterData&apiKey=${API_KEY}&t=${Date.now()}`);
...
// ===============================================================
// LOAD DATA DARI CLOUD (DENGAN API KEY)
// ===============================================================
function loadDataFromCloud() {
    ...
    fetch(`${SCRIPT_URL}?action=read&apiKey=${API_KEY}`)
        .then(response => response.json())
        .then(cloudData => {
            if (!cloudData || cloudData.error) throw new Error(cloudData.error || "Data tidak valid");
            isLoading = false;
...
// ===============================================================
// FUNGSI POST MENU / LAYANAN (DENGAN API KEY)
// ===============================================================
function saveNewService() {
    ...
    if (SCRIPT_URL !== "" && !SCRIPT_URL.includes("SCRIPT_URL")) {
        const payloadToSend = { action: "addService", apiKey: API_KEY, id: newId, name, price, type };
...
function submitEditService() {
    ...
    if (idx !== -1) {
        ...
        if (SCRIPT_URL !== "" && !SCRIPT_URL.includes("TEMPEL_URL")) {
            const editPayload = { action: "editService", apiKey: API_KEY, id, name, price, type };
...
function deleteServiceFromPOS(id) {
    ...
    if (confirm(`Apakah Anda yakin ingin menghapus layanan "${match.name}"?`)) {
        ...
        if (SCRIPT_URL !== "" && !SCRIPT_URL.includes("TEMPEL_URL")) {
            const deletePayload = { action: "deleteService", apiKey: API_KEY, id };
...
// ===============================================================
// FUNGSI CHECKOUT DENGAN API KEY
// ===============================================================
function processCheckout() {
    ...
    if (isNewCustomerMode || customers.length === 0) {
        ...
        if (SCRIPT_URL !== "" && !SCRIPT_URL.includes("TEMPEL_URL")) {
            const custPayload = { action: "addCustomer", apiKey: API_KEY, id: newCustId, name: customerName, phone: customerPhone };
            fetch(SCRIPT_URL, {
    ...
    const checkoutPayload = {
        action: "checkout",
        apiKey: API_KEY, // PENJAGA GERBANG API
        id: generatedOrderId, 
    ...
    orders.unshift({ ...checkoutPayload, itemsDetail: JSON.stringify(itemsDetailBackup) });
    renderOrders(); calculateFinance(); openReceiptModal(checkoutPayload);

    if (SCRIPT_URL !== "" && !SCRIPT_URL.includes("SCRIPT_URL")) {
        fetch(SCRIPT_URL, { method: 'POST', mode: 'no-cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...checkoutPayload, itemsDetail: JSON.stringify(itemsDetailBackup) }) })
...
function openReceiptModal(order) {
    ...
    if (notaItemsContainer) {
        ...
        if (order.itemsDetail) {
            try { itemsToRender = typeof order.itemsDetail === 'string' ? JSON.parse(order.itemsDetail) : order.itemsDetail; } catch (e) { itemsToRender = []; }
        }

        if (itemsToRender && itemsToRender.length > 0) {
            notaItemsContainer.innerHTML = itemsToRender.map(item => {
                let unit = 'x';
                if (item.type) {
...
// ===============================================================
// FUNGSI UPDATE DATA DENGAN API KEY
// ===============================================================
function updatePaymentMethod(orderId, newMethod) {
    ...
    if (orderIndex !== -1) {
        ...
        if (SCRIPT_URL !== "" && !SCRIPT_URL.includes("SCRIPT_URL")) {
            fetch(SCRIPT_URL, { method: 'POST', mode: 'no-cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: "updatePaymentMethod", apiKey: API_KEY, id: orderId, method: newMethod }) }).catch(err => console.log(err));
        }
...
function updateOrderStatus(orderId, newStatus) {
    ...
    if (orderIndex !== -1) {
        ...
        if (SCRIPT_URL !== "" && !SCRIPT_URL.includes("SCRIPT_URL")) {
            fetch(SCRIPT_URL, { method: 'POST', mode: 'no-cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: "updateStatus", apiKey: API_KEY, id: orderId, status: newStatus }) }).catch(err => console.log(err));
        }
...
function updatePaymentStatus(orderId, newPaymentStatus) {
    ...
    if (orderIndex !== -1) {
        ...
        if (SCRIPT_URL !== "" && !SCRIPT_URL.includes("SCRIPT_URL")) {
            fetch(SCRIPT_URL, { method: 'POST', mode: 'no-cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: "updatePaymentStatus", apiKey: API_KEY, id: orderId, paymentStatus: newPaymentStatus }) }).catch(err => console.log("Gagal sinkron cloud:", err));
        }
...
window.addEventListener('load', () => {
    ...
    if (orderParam) {
        ...
        function fetchStatusPelanggan() {
            ...
            fetch(`${SCRIPT_URL}?order=${encodeURIComponent(orderParam)}&apiKey=${API_KEY}`) // TAMBAH API KEY
                .then(response => response.json())
                .then(cloudData => {
...
function calculateFinance() {
    ...
    const tIncome = filteredOrders.reduce((sum, o) => sum + parseNominal(o.total), 0); 
    const tLunas = lunasOrders.reduce((sum, o) => sum + parseNominal(o.total), 0); 
    const tBelumLunas = piutangOrders.reduce((sum, o) => sum + parseNominal(o.total), 0); 
    const tExpense = filteredExpenses.reduce((sum, e) => sum + parseNominal(e.nominal || e.amount), 0);

    const netProfit = tLunas - tExpense; 
...
// ===============================================================
// FUNGSI POST PENGELUARAN (DENGAN API KEY)
// ===============================================================
function saveNewExpense() {
    ...
    const payload = {
        action: "addExpense",
        apiKey: API_KEY, // TAMBAHKAN API KEY
...
function deleteExpense(expId) {
    ...
    if (SCRIPT_URL !== "" && !SCRIPT_URL.includes("SCRIPT_URL")) {
        fetch(SCRIPT_URL, { 
            method: 'POST', mode: 'no-cors', headers: { 'Content-Type': 'application/json' }, 
            body: JSON.stringify({ action: "deleteExpense", apiKey: API_KEY, id: expId }) 
...
async function printBluetoothClosing() {
    ...
    try {
        ...
        const uangFisikLaci = tTunai - tExpenseTunai; 
